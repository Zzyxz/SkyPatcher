			...
			for (const auto& curobj : objectSpellArray) {
				...
				if (found && !line.addedObjects.empty()) {
					for (const auto& objectToAdd : line.addedObjects) {
						...
						logger::debug(FMT_STRING("spell {:08X} {} added effect {:08X} {} magitude {} duration {} area {}"), curobj->formID, curobj->fullName, addForm->formID, addForm->fullName, objectToAdd[1], objectToAdd[2], objectToAdd[3]);
					}
				}
					...
					for (const auto& objectToRemove : line.removedObjects) {
						...
						if (removeForm) {
							curobj->effects.erase(std::remove_if(curobj->effects.begin(), curobj->effects.end(), [&](const RE::Effect* x) {
								...
								if (removed) {
									logger::debug(FMT_STRING("spell {:08X} {} removed effect {:08X}"), curobj->formID, curobj->fullName, removeForm->formID);
								}
								return removed;
							}));
				...
				if (found && !line.capsvalue.empty() && line.capsvalue != "none") {
					try {
						...
						logger::debug(FMT_STRING("spell formid: {:08X} {} changed value {}"), curobj->formID, curobj->fullName, curobj->data.costOverride);
